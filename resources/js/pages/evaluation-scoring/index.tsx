...
import { index as scoringIndex, create as scoringCreate, edit as scoringEdit } from '@/routes/evaluation-scoring';
            ...
            <div className="space-y-6 p-4">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Mostrando {evaluations.from ?? 0} a {evaluations.to ?? 0} de {evaluations.total}
                        </CardDescription>
                    </CardHeader>
                                                        ...
                                                        <Link
                                                            href={
                                                                item.answers_count > 0
                                                                    ? scoringEdit(item.id)
                                                                    : scoringCreate({
                                                                          query: { evaluation_id: item.id },
                                                                      })
                                                            }
                                                        >
                                                            {item.answers_count > 0 ? 'Editar calificación' : 'Iniciar calificación'}
                                                        </Link>
                                                    </Button>
                                                </div>
                                            </td>
...
EvaluationScoringIndex.layout = {
    breadcrumbs: [{ title: 'Formulario de calificación', href: scoringIndex() }],
};
