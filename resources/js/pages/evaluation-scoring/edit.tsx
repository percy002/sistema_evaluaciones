...
export default function EvaluationScoringEdit({
    evaluation,
    rows,
    answers,
    summary,
    ...
    const submit = (event: FormEvent<HTMLFormElement>) => {
        ...
        setClientError(null);
        patch(EvaluationScoringController.update.url(evaluation.id));
    };

    return (
        <>
            <Head title="Editar calificación" />
            ...
            <div className="space-y-6 p-4">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Promedios calculados desde las respuestas guardadas.</CardDescription>
                    </CardHeader>
                                        ...
                                        <tbody>
                                            {summary.by_category.map((item) => (
                                                <tr
                                                    ...
                                                    className="border-b last:border-none hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                                                >
                                    ...
                                    <table className="w-full min-w-120 text-sm">
                                        ...
                                        <tbody>
                                            {summary.by_competency.map((item, index) => (
                                                <tr
                                                    key={item.competency_id ?? `${item.competency_name ?? 'competency'}-${index}`}
                                                    className="border-b last:border-none hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                                                >
                    ...
                    <CardContent>
                        <form onSubmit={submit} className="space-y-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <Button type="submit" disabled={processing}>Actualizar calificación</Button>
                                <Button variant="outline" asChild>
                                    <Link href={scoringIndex()}>Cancelar</Link>
                                </Button>
                                <Button variant="destructive" asChild>
                                    <Link href={EvaluationScoringController.destroy(evaluation.id)} method="delete" as="button">
                                        Reiniciar
                                    </Link>
                                </Button>
                            </div>
...
EvaluationScoringEdit.layout = {
    breadcrumbs: [
        { title: 'Formulario de calificación', href: scoringIndex() },
        { title: 'Editar calificación', href: scoringIndex() },
    ],
};
